= Use {hazelcast-cloud} Serverless as a Write-Through Cache with MongoDB Atlas
:page-product: cloud
...
+
TIP: Make a note of these credentials because you will need to give them to {hazelcast-cloud} Serverless later.
...
+
Your {hazelcast-cloud} Serverless cluster will store your data in this *person* collection.
+
...
Now your database is ready. Your {hazelcast-cloud} Serverless cluster will be able to connect to it and write data to your collection.
...
The sample code for this tutorial is in the link:{github-directory}/src/main/java/sample/com/hz/demos/mapstore/mongo[`src/main/java/sample/com/hz/demos/mapstore/mongo/`] directory:
...
link:{github-directory}/src/main/java/sample/com/hz/demos/mapstore/mongo/Person.java[View source]
====
...
link:{github-directory}/src/main/java/sample/com/hz/demos/mapstore/mongo/MongoPersonRepository.java[View source]
====
...
The `init()` method reads the database connection credentials from the Java properties. You'll provide these in the Hazelcast {hazelcast-cloud} console when you configure the MapStore.
...
link:{github-directory}/src/main/java/sample/com/hz/demos/mapstore/mongo/MongoPersonMapStore.java[View source]
====

[[step-three]]
== Step 3. Deploy the Classes to the Cluster

In this step, you'll use the xref:cloud:ROOT:maven-plugin-hazelcast.adoc[{hazelcast-cloud} Serverless Maven plugin] to package the project into a single JAR file and upload that file to your cluster.
...
[cols="1a,1a"]
|===
|Element|Location in the Hazelcast {hazelcast-cloud} console
...
. Execute the following goal of the Maven plugin to package the project into a JAR file and deploy that file to your {hazelcast-cloud} Serverless cluster:
+
```bash
...
. Go to *Data Structure Config* > *Add New Configuration* in the  Hazelcast {hazelcast-cloud} console.
. Enter *person* in the *Map Name* field.
. In the bottom left corner, select *Enable MapStore*.
. Select the *sample.com.hazelcast.cloud.mapstore5.mongo.MongoPersonMapStore* classname from the dropdown. If you don't see a classname, you need to <<step-three, deploy the MapStore to your {hazelcast-cloud} Serverless cluster>>.
...
. Sign into the Hazelcast {hazelcast-cloud} console.
...
- Use a Mapstore to write changes made in a {hazelcast-cloud} Serverless cluster back to a MongoDB Atlas database.
- Deploy the MapStore to a {hazelcast-cloud} Serverless cluster, using the Hazelcast {hazelcast-cloud} Maven plugin.
